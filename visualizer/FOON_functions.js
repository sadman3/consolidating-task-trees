...
function drawD3Graph(graph, S) {
	...
	var node = svg.append("g")
		...
		.append("circle")
		.attr("r", 16) // for regular graph images
		//.attr("r", 28) // for universal FOON images
		.style("fill", function(d) { 
			console.log('value ',d)
			if (d.id == "prunes@states:@\t<dried>@\t<in [mixing bowl]>" || d.id == "prunes@states:@\t<dried>@\t<in [bowl]>") {
				console.log('hello hi helo')
				return "blue"
			}
			else if (d.type == 1) return "#99FF55"; else if (d.type == 2) return "#E74C3C";}) // #adff2f
		.call(d3.drag()
	...
	text.append("text")
		...
		.each(function (d) {
			...
			for (var i = 0; i < lines.length; i++) {
				d3.select(this)
					.append("tspan")
					.attr("dy", function (d) { if (i == 0) return square_length+3; return separation+3; })
					.attr("x", textX)
					...
					.style("font-weight", function () { if (i <=1) return "bold"; return "semi-bold"; })
					.style("font-size", function (d) { if (d.type == "1" && (lines[i].includes(":") || lines[i].includes("<"))) return "1.2em"; return "1.3em" }) // -- original!
					//.style("font-size", "0em")
					.text(function(d){
						if (!lines[i].includes("state")) return lines[i]
					});
			}
		});
